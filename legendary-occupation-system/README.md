...
The System isn't a message log. It is an **AI companion with a conscience**:

- **It thinks.** Its reasoning is visible in-game as a dimmed *SYSTEM CORE · reasoning* trace before it speaks.
- **It judges.** A moral ledger tracks your kindness and your greed. Return a lost phone and it is proud of you; pawn one and it will bring it up later. It never celebrates a profitable wrong.
...
| **Full mind** | A live Claude model (`claude-opus-5` by default) with adaptive thinking and refusal fallbacks enabled | CLI: `npm install` + `ANTHROPIC_API_KEY` env var. Web: paste a key under **⚙ Mind & Save** |

Override the model with `LOS_MODEL` (CLI) or the model selector (web). Keys pasted in the web version stay in your browser's local storage and are sent only to `api.anthropic.com`.
...
```bash
npm test           # 34 deterministic tests (engine, content integrity, mind, build)
...
src/mind.js      SystemMind: persona, prompts, moral ledger, offline shard
src/mind-node.js Claude backend for the CLI (official @anthropic-ai/sdk)
